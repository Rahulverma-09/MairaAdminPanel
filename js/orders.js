...
async function loadOrdersData() {
    ...
    try {
        if (typeof API !== 'undefined' && API.getOrders) {
            ...
            if (res && res.data) {
                ordersList = Array.isArray(res.data) ? res.data : (res.data.orders || []);
...
function renderOrders() {
    ...
    tbody.innerHTML = paginated.map(order => {
        ...
        const customer = order.customer || { name: 'Customer', email: '—', phone: '—', address: '—' };
        const totalAmount = order.total != null ? order.total : (order.subtotal || 0);

        return `
            <tr>
                <td><span class="order-code">${escapeHtml(orderId)}</span></td>
                <td>
                    <button type="button" class="btn-link table-clickable-cell" data-customer-order="${orderId}" title="View customer profile">
                        ...
                        <span>${escapeHtml(customer.name || 'Anonymous Customer')}</span>
                    </button>
                </td>
                <td>
                    <button type="button" class="table-item-pill" data-items-order="${orderId}" title="Click to view ordered items">
                        ${itemCount} ${itemCount === 1 ? 'item' : 'items'}
                    </button>
                </td>
                <td><strong style="color: var(--color-charcoal); font-weight: 700; font-size: 0.92rem;">${formatPrice(totalAmount)}</strong></td>
                <td>
                    <select class="input input--select order-status-select" data-order-id="${orderId}">
                ...
                <td>
                    <div class="actions">
                        <button class="btn btn--sm btn--secondary" data-view-order="${orderId}">View</button>
                        <button class="btn btn--sm btn--danger" data-delete-order="${orderId}">Delete</button>
                    </div>
                </td>
            </tr>
        `;
...
function showCustomerModal(order) {
    ...
    const custId = customer.id || customer._id || ('CUST-' + (order.id || order._id || '').replace(/[^0-9]/g, ''));
    const orderId = order.id || order._id || 'N/A';
    ...
    body.innerHTML = `
        <div style="text-align: center; margin-bottom: 1.25rem; padding-bottom: 1rem; border-bottom: 1px solid #f3ede2;">
            ...
            <h4 style="font-family: var(--font-serif); font-size: 1.2rem; color: var(--color-charcoal); margin-bottom: 0.2rem;">${escapeHtml(customer.name || 'Customer')}</h4>
        ...
        <div class="detail-section">
            ...
            <div class="detail-row"><span class="detail-row__label">Address</span><span class="detail-row__value" style="max-width: 250px; text-align: right;">${escapeHtml(customer.address || customer.shippingAddress || '—')}</span></div>
    ...
    if (items.length === 0) {
        ...
        body.innerHTML = `
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 1rem; border-top: 1px solid #f3ede2;">
                <span style="font-weight: 600; color: var(--color-muted); font-size: 0.85rem;">Total Items: ${items.reduce((s, i) => s + (i.quantity || 1), 0)}</span>
                <span style="font-weight: 700; color: var(--color-charcoal); font-size: 1.1rem;">Total: ${formatPrice(order.total || order.subtotal || 0)}</span>
            </div>
        `;
    }
...
function showOrderDetails(order) {
    ...
    const itemsHtml = items.map(item => `
        <div class="order-item">
            ...
            <div class="order-item__info">
                ...
                <div class="order-item__meta">Qty: ${item.quantity || 1} · ${escapeHtml(item.price || formatPrice(item.priceNum || 0))}</div>
            </div>
            <div class="order-item__price">${formatPrice((item.priceNum || parseFloat(item.price) || 0) * (item.quantity || 1))}</div>
        </div>
    `).join('');
    ...
    body.innerHTML = `
        <div class="detail-section">
            ...
            <div class="detail-row"><span class="detail-row__label">Date</span><span class="detail-row__value">${formatDate(order.date || order.createdAt)}</span></div>
        ...
        <div class="detail-section">
            ...
            <div class="detail-row"><span class="detail-row__label">Address</span><span class="detail-row__value">${escapeHtml(customer.address || customer.shippingAddress || '—')}</span></div>
        </div>
        <div class="detail-section">
        ...
        <div class="detail-section">
            ...
            <div class="detail-row"><span class="detail-row__label">Subtotal</span><span class="detail-row__value">${formatPrice(order.subtotal || order.total || 0)}</span></div>
            <div class="detail-row"><span class="detail-row__label">Tax</span><span class="detail-row__value">${formatPrice(order.tax || 0)}</span></div>
            <div class="detail-row"><span class="detail-row__label">Total</span><span class="detail-row__value" style="color: #9e7f47; font-size: 1.1rem; font-weight: 700;">${formatPrice(order.total || order.subtotal || 0)}</span></div>
        </div>
    `;
